...
class BeautifulDoc(BaseDocTemplate):
    ...
    def _decorate(self, canv, doc):
        ...
        # Use STHeiti for header if available (supports CJK), else fall back
...
def make_styles(t: dict) -> dict:
    ...
    # ── CJK font override ───────────────────────────────────────────────────
    # Use registered STHeiti (macOS 黑体) for Chinese content; fall back to
    # the token font if CJK fonts were not registered (non-macOS environments).
    ...
    # All headings also use STHeiti to support CJK characters
    _cjk_display = _cjk_bold

    return {
        "h1": ParagraphStyle("H1",
            fontName=_cjk_display, fontSize=t["size_h1"],
            leading=t["size_h1"] * 1.3,
            textColor=HexColor(d),
            spaceBefore=t["section_gap"], spaceAfter=4,
        ),
        "h2": ParagraphStyle("H2",
            fontName=_cjk_display, fontSize=t["size_h2"],
            leading=t["size_h2"] * 1.4,
            textColor=HexColor(d),
            spaceBefore=18, spaceAfter=5,
        ),
        "h3": ParagraphStyle("H3",
            fontName=_cjk_bold, fontSize=t["size_h3"],
            leading=t["size_h3"] * 1.5,
            textColor=HexColor(d),
            spaceBefore=12, spaceAfter=3,
        ),
        "body": ParagraphStyle("Body",
            fontName=_cjk_regular, fontSize=t["size_body"],
            leading=t["line_gap"],
        ...
        "callout": ParagraphStyle("Callout",
            fontName=_cjk_bold, fontSize=t["size_body"] + 0.5, leading=16,
            textColor=HexColor(d),
        ),
        ...
        "table_header": ParagraphStyle("TblH",
            fontName=_cjk_bold, fontSize=9.5, leading=13,
            textColor=HexColor("#FFFFFF"),
        ),
        "table_cell": ParagraphStyle("TblC",
            fontName=_cjk_regular, fontSize=9.5, leading=13,
            textColor=HexColor(dk),
        ),
